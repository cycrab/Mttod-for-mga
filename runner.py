...
from transformers import AdamW, get_linear_schedule_with_warmup, get_constant_schedule,get_polynomial_decay_schedule_with_warmup
...
class BaseRunner(metaclass=ABCMeta):
    ...
    def get_optimizer_and_scheduler(self, num_traininig_steps_per_epoch, train_batch_size):
        ...
        elif self.cfg.add_residual_lr:
            scheduler = get_polynomial_decay_schedule_with_warmup(
                optimizer,
                num_warmup_steps=num_warmup_steps,
                num_training_steps=num_train_steps,
                lr_end=self.cfg.residual_lr)
        else:
...
class MultiWOZRunner(BaseRunner):
    ...
    def train_epoch(self, train_iterator, optimizer, scheduler, reporter=None):
        ...
        for step, batch in enumerate(train_iterator):
            ...
            inputs, labels = batch
            if self.count>0:
                print(self.reader.tokenizer.decode(inputs[0]))
    ...
    def train(self):
        ...
        for epoch in range(1, self.cfg.epochs + 1):
            train_iterator = self.iterator.get_data_iterator(
                train_batches, self.cfg.task, self.cfg.ururu, self.cfg.simplify,self.cfg.add_auxiliary_task, self.cfg.context_size)
    ...
    def validation(self, global_step):
        ...
        dev_iterator = self.iterator.get_data_iterator(
            dev_batches, self.cfg.task, self.cfg.ururu,self.cfg.simplify, self.cfg.add_auxiliary_task, self.cfg.context_size)
    ...
    def predict(self):
        ...
        for dial_batch in tqdm(pred_batches, total=len(pred_batches), desc="Prediction"):
            ...
            dial_history_simplified=[[] for _ in range(batch_size)]
            ...
            for turn_batch in self.iterator.transpose_batch(dial_batch):
                ...
                for t, turn in enumerate(turn_batch):
                    ...
                    if self.cfg.simplify:
                        encoder_input_ids = dial_history_simplified[t] + turn["user"] + [self.reader.eos_token_id]
                    else:
                ...
                for t, turn in enumerate(turn_batch):
                    ...
                    if self.cfg.ururu:
                        ...
                        pv_text_simplified = pv_bspn+ pv_resp
                    dial_history[t].append(pv_text)
                    dial_history_simplified[t]=pv_text_simplified
            ...
            for dial_id,dial in result.items():
                ...
                simplified_turn={}
                for turn in dial:
                    simplified_turn['turn_domain']=turn['turn_domain']
                    simplified_turn.update(turn['user_span'])
                    simplified_dial.append(simplified_turn)
                simplified_result[dial_id] = simplified_dial
            simplified_results.update(**simplified_result)
            """
